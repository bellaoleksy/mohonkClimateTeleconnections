...
MohonkWeeklyProfilesMetric.derivedData$year<-year(MohonkWeeklyProfilesMetric.derivedData$Date)
...
for(iii in 1:length(MohonkWeeklyProfilesMetric.derivedData$Date)){
  ...
  if(sum(!is.na(MohonkWeeklyProfilesMetric.derivedData[iii,3:16]))<3){}else{
    #Call thermocline function
    MohonkWeeklyProfilesMetric.derivedData$thermoclineDepth_m_thresh0.1[iii]<-thermocline.Depth(depths.v,MohonkWeeklyProfilesMetric.derivedData[iii,3:16],0.1)
    #Call thermocline function
    MohonkWeeklyProfilesMetric.derivedData$thermoclineDepth_m_maxdiff[iii]<-thermocline.Depth.max(depths.v,MohonkWeeklyProfilesMetric.derivedData[iii,3:16])
    #call stability function
    MohonkWeeklyProfilesMetric.derivedData$stability_Jperm2[iii]<-stability.calc(as.numeric(MohonkWeeklyProfilesMetric.derivedData[iii,3:16]),depths.v,MohonkBathy$SurfaceAreaAtThatDepth_m2,MohonkBathy$Depth_m_LowerLimit)
    #call stability function
    MohonkWeeklyProfilesMetric.derivedData$buoyancyfrequency_1_s2[iii]<-buoyancy.freq.profile.max(MohonkWeeklyProfilesMetric.derivedData[iii,3:16],depths.v)
...
VarsOfInterest<-names(MohonkWeeklyProfilesMetric.derivedData.fullYears)[c(1,3:16,17:21)]
...
for(index.l in 1:length(DailyInterpol$Date)){
  ...
  tmp.profile<-DailyInterpol[index.l,2:15]
  ...
  if(sum(!is.na(tmp.profile))<8){DailyInterpol$VolumeWeightedMeanTemp_degC[index.l]<-NA}else{
    ...
    cx <- c(0, cumsum(ifelse(is.na(DailyInterpol[index.l,2:15]), 0, DailyInterpol[index.l,2:15])))
    cn <- c(0, cumsum(ifelse(is.na(DailyInterpol[index.l,2:15]), 0, 1)))
...
AnnualData<-left_join(AnnualData,MohonkIcePost1985%>%
                        ...
                        mutate(Year=Year-1)%>%
                        mutate(IceInDayofYear=if_else(IceInDayofYear<35,365+IceInDayofYear,IceInDayofYear)))%>%
  filter(!Year=="2019")
...
for(k in 1:length(AnnualData$Year)){
  ...
  deltaSurfaceDeep.tmp<-DailyInterpol[DailyInterpol$year==year.tmp,c("DeltaSurfaceDeep_degC")]
  ...
  if(year.tmp==1984|year.tmp==1997|year.tmp==2014){}else{
    ...
    dayofyear.tmp.openwater<-dayofyear.tmp[dayofyear.tmp>StartOpenWater_doy&dayofyear.tmp<EndOpenWater_doy]
    stability.tmp.openwater<-stability.tmp[dayofyear.tmp>StartOpenWater_doy&dayofyear.tmp<EndOpenWater_doy]
    ...
    AnnualData$AUC.Stability.openwater_Jperm2day[k]<-sum(diff(dayofyear.tmp.openwater[!is.na(stability.tmp.openwater)]) * (head(stability.tmp.openwater[!is.na(stability.tmp.openwater)],-1)+tail(stability.tmp.openwater[!is.na(stability.tmp.openwater)],-1))/2)
  ...
  #Use ice off (IceOutDayofYear) to StartOfStratification_Day
    ...
    AnnualData$DeltaSurfaceDeepTemp_slope_degCpDay[k]<-lm.deltaSpringMix$coefficients[2,"Estimate"]
    AnnualData$DeltaSurfaceDeepTemp_intercept_degC[k]<-lm.deltaSpringMix$coefficients[1,"Estimate"]
    }
  }
#End of for loop through the years
...
ENSO_MEI_seasonal <- ENSO_MEI_monthly %>%
  #Adds a category for season
  mutate(Season =
           ifelse(month %in% c(4, 5, 6), "spring",
                  ifelse(month %in% c(7, 8, 9), "summer",
                         ifelse(month %in% c(1, 2, 3), "winter-year", "winter-previous")))) %>%
  # filter(Season %in% c("spring","summer"))%>%
  rename(Year=year) %>%
  group_by(Year,Season) %>%
  mutate(MEI_mean=mean(ENSO_MEI, na.rm=TRUE),
         MEI_sum=sum(ENSO_MEI, na.rm=TRUE))%>%
  select(Year, Season, MEI_mean, MEI_sum) %>%
  group_by(Year,Season)%>%
  summarize_at(vars(MEI_mean,MEI_sum), mean, na.rm=TRUE) 

ENSO_MEI_winteryear<-ENSO_MEI_seasonal %>%
  filter(Season=="winter-year") %>%
  rename(MEI_mean_winteryear=MEI_mean,
         MEI_sum_winteryear=MEI_sum)%>%
  select(-Season)
ENSO_MEI_winterpreviousyear<-ENSO_MEI_seasonal %>%
  filter(Season=="winter-previous") %>%
  mutate(Year=Year+1) %>%
  rename(MEI_mean_previouswinter=MEI_mean,
         MEI_sum_previouswinter=MEI_sum) %>%
  select(-Season)

ENSO_MEI_winter<-left_join(ENSO_MEI_winteryear,ENSO_MEI_winterpreviousyear, by="Year") %>%
      group_by(Year)%>%
      mutate(ENSO_MEI_Winter_mean=(MEI_mean_winteryear+MEI_mean_previouswinter)/2,
             ENSO_MEI_Winter_sum=MEI_sum_winteryear+MEI_sum_previouswinter) %>%
      select(-(2:5))


ENSO_MEI_spring <- ENSO_MEI_seasonal %>%
  filter(Season=="spring")%>%
  rename(ENSO_MEI_Spring_mean=MEI_mean,
         ENSO_MEI_Spring_sum=MEI_sum) %>%
  select(-Season)

ENSO_MEI_summer <- ENSO_MEI_seasonal %>%
  filter(Season=="summer")%>%
  rename(ENSO_MEI_Summer_mean=MEI_mean,
         ENSO_MEI_Summer_sum=MEI_sum) %>%
  select(-Season)


ENSO_MEI_summary <- left_join(ENSO_MEI_spring,ENSO_MEI_summer, by="Year" )
...
#*Secchi to annual summary
  ...
  DailyInterpol.secchiPeriods<-DailyInterpol.secchi%>%
      ...
      left_join(.,AnnualData%>%dplyr::select(Year,StartOfStratification_Day,EndOfStratification_Day,IceOutDayofYear),by="Year")%>%
...
#*VolumeWeightedMeans to annual summary
  ...
  AnnualData_volWtMeansPeriods<-DailyInterpol.volWtMeansPeriods%>%
    group_by(Year)%>%
    summarize(VolumeWeightedMeanTemp_Summer_degC=mean(SummerPeriod,na.rm=TRUE),
              VolumeWeightedMeanTemp_StratifiedPeriod_degC=mean(StratifiedPeriod,na.rm=TRUE),
              VolumeWeightedMeanTemp_Spring_degC=mean(SpringPeriod,na.rm=TRUE),
              VolumeWeightedMeanTemp_SpringMixed_degC=mean(SpringMixedPeriod,na.rm=TRUE),
              VolumeWeightedMeanTemp_SpringPostIce_degC=mean(PostIcePeriod,na.rm=TRUE))
  #**Merge with AnnualData
...
#*Air temperature to annual summary####
  ...
  DailyInterpol.precip<-MohonkDailyWeatherTruncate%>%
    ...
    left_join(.,AnnualData%>%
                dplyr::select(Year,StartOfStratification_Day,
                              EndOfStratification_Day,IceOutDayofYear),
              by="Year")%>%
    mutate(SummerPeriod=ifelse(dayOfYear>=StartOfSummer&dayOfYear<EndOfSummer,Precip_mm,NA))%>%
    mutate(StratifiedPeriod=ifelse(dayOfYear>=StartOfStratification_Day&dayOfYear<EndOfStratification_Day,Precip_mm,NA))%>%
    mutate(SpringPeriod=ifelse(dayOfYear>StartOfSpring&dayOfYear<StartOfSummer,Precip_mm,NA))%>%
    mutate(SpringMixedPeriod=ifelse(dayOfYear>IceOutDayofYear&dayOfYear<=StartOfStratification_Day,Precip_mm,NA))%>%
    mutate(PostIcePeriod=ifelse(dayOfYear>IceOutDayofYear&dayOfYear<=(IceOutDayofYear+7),Precip_mm,NA))
  
  #**Summarize the 5 different precip periods by year - summation
  AnnualData_precipPeriods<-DailyInterpol.precip%>%
    group_by(Year)%>%
    summarize(Precip_Summer_mm=sum(SummerPeriod,na.rm=TRUE),
              Precip_StratifiedPeriod_mm=sum(StratifiedPeriod,na.rm=TRUE),
  ...
  AnnualData_precipPeriods<-AnnualData_precipPeriods%>%mutate(Precip_StratifiedPeriod_mm=na_if(Precip_StratifiedPeriod_mm, 0),Precip_SpringMixed_mm=na_if(Precip_SpringMixed_mm, 0))
  
  #**Merge with AnnualData
  AnnualData<-left_join(AnnualData,AnnualData_precipPeriods,by="Year")
